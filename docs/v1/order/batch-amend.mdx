...
- A maximum of 10 orders (spot) can be amended per request.
:::

### HTTP Request
POST `/v1/order/amend-batch`
...
|> triggerPrice |false |string |<ul><li>For spot, it is for tpslOrder or stopOrder trigger price</li></ul>|
...
```http
POST /v1/order/amend-batch HTTP/1.1
