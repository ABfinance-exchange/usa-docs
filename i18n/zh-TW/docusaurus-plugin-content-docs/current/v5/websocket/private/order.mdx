...
|data |array |Object |
|> category |string |產品類型 `spot`|
|> orderId  |string |訂單ID |
|> orderLinkId |string |用戶自定義ID |
|> isLeverage |string |是否借貸. 僅`spot`有效<li>`0`: 否, 幣幣交易</li><li> `1`: 是, 槓桿交易</li>|
...
|> cumExecFee |string |<li> `spot`: 已棄用. 用`cumFeeDetail`替代.</li><li>升級到統一帳戶後, 您可以使用[成交](execution)頻道中的`execFee`字段來獲取每次成交的手續費</li> |
...
|> cumFeeDetail |json |<li>`spot`: 累積交易費詳情, 替代`cumExecFee`</li>` |
