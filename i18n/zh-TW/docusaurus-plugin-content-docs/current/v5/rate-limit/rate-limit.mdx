...
## IP限頻
### HTTP IP限頻
默認情況下, 每個IP允許在每5秒的時間窗口內發送最多600次請求。這個速率限制將統計所有打到`api.abfinance.com`域名的請求數量。
...
### Websocket IP 限頻
* 不要在5分鐘內構建超過500個連接, 這個限頻適用於所有發往`stream.abfinance.com` 域名的連接請求;
* 不要嘗試頻繁地構建連接與斷開連接;
* 訂閱行情數據時, 每個IP不要構建超過1,000個連接。

## 賬戶頻率限製
:::caution
如果您收到這樣的響應`"retCode": 10006, "retMsg": "Too many visits!"`, 則表示您觸發了帳戶頻率限制, 請等到頻率限制重置以後, 再繼續發送請求。
:::
平台基於**每秒鍾**的滾動時間窗口來做頻率限製，並且是按**賬戶**（uid）來做劃分限製，每次請求API響應頭(response header)中都會包含如下字段：
...
<table border="0.8">
    <tr>
        <th rowSpan="2">請求方式</th><th rowSpan="2">路徑</th><th align="center">統一帳戶</th><th rowSpan="2">是否可提頻</th>
    </tr>
    <tr>
        <th>現貨</th>
    </tr>
...
<table border="0.8">
    ...
    <tr>
        <td>/v5/account/fee-rate</td><td>category=spot</td><td>5/s</td><td align="center">N</td>
    </tr>
